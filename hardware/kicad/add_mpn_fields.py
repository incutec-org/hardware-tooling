...
Reads LCSC/LCSC_Part properties from explicitly selected `.kicad_sch` files,
optionally looks up the MPN and manufacturer via the jlcsearch API, and writes
them back as new symbol properties using kicad-skip.

Usage:
    python3 add_mpn_fields.py hardware/*.kicad_sch
    python3 add_mpn_fields.py --lookup hardware/*.kicad_sch
    python3 add_mpn_fields.py --lookup --write hardware/*.kicad_sch

Network access and schematic writes are independent opt-ins. Without
`--lookup`, only an existing cache is used. Without `--write`, schematics are
not changed.
"""
...
def process_schematic(
    sch_path: Path, cache: dict, write: bool, lookup: bool
) -> tuple[int, int]:
    ...
    for sym in s.symbol:
        ...
        if not lookup and lcsc not in cache:
            skipped += 1
            continue

        if lookup and lcsc not in cache:
...
def main():
    parser = argparse.ArgumentParser(description="Add MPN/Manufacturer fields to selected KiCad schematics")
    parser.add_argument(
        "schematics",
        nargs="+",
        type=Path,
        help="explicit .kicad_sch files to inspect",
    )
    parser.add_argument("--write", action="store_true", help="Write changes (default is dry-run)")
    parser.add_argument("--lookup", action="store_true", help="Allow jlcsearch API lookups")
    parser.add_argument(
        "--cache",
        type=Path,
        default=Path(".lcsc-mpn-cache.json"),
        help="cache file (default: ./.lcsc-mpn-cache.json)",
    )
    args = parser.parse_args()

    sch_files = [path.resolve() for path in args.schematics]
    bad = [path for path in sch_files if path.suffix != ".kicad_sch" or not path.is_file()]
    if bad:
        parser.error("not a readable .kicad_sch file: " + ", ".join(map(str, bad)))

    print(f"Inspecting {len(sch_files)} explicitly selected schematic files")
    if not args.write:
    ...
    for f in sch_files:
        print(f"\nProcessing {f.name}...")
        modified, skipped = process_schematic(f, cache, args.write, args.lookup)
        total_modified += modified
        total_skipped += skipped

    if args.lookup and cache != load_cache(cache_file):
        save_cache(cache_file, cache)

    print(f"\n{'WROTE' if args.write else 'Would modify'} {total_modified} symbols")
    if total_skipped:
        print(f"Skipped {total_skipped} symbols (no MPN data)")
    print(f"Cache: {len(cache)} entries (was {initial_cache_size}) at {cache_file}")
